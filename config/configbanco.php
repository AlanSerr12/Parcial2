...
class ClaseConexion{
    ...
    private $baseBanco = 'bancoo';//TODO: Variable del base de datos

//TODO: Procedimiento para conectar a la base de datos
    public function ProcedimientoConectar(){
        $this->conexion = mysqli_connect($this->host,$this->usuario,$this->password, $this->baseBanco);
        mysqli_query($this->conexion,"SET NAMES utf8");
        if($this->conexion == 0){
            die('error al conectarse al servidor' . mysqli_error($this->conexion));
        }
        $this->db= mysqli_select_db($this->conexion, $this->baseBanco);
        if($this->db == 0){
